            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                GroupBox("Компактное окно перевода") {
                    VStack(alignment: .leading, spacing: 14) {
                        Text(
                            "Когда приложение неактивно и перевод запущен через shortcut, снизу по центру появится маленькое окно. Пока идёт обработка, окно остаётся до ручного закрытия (кнопка ✕ или Escape). Готовый перевод скрывается по таймеру: максимум из минимального времени и «количество слов × секунд на слово», если для сессии не включено «Показывать перевод слов в компактном окне» — тогда окно остаётся открытым, пока вы его не закроете, и может показать перевод слов под основным текстом. Последний перевод можно открыть отдельной горячей клавишей и держать на экране, пока вы его сами не закроете."
                        )
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
